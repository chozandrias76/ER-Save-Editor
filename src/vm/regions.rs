pub mod regions_view_model {
    ...
    impl RegionsViewModel {
        ...
        pub fn open_region_file_dialog() -> Option<PathBuf> {
            Self::region_file_dialog().pick_file()
        }

        pub fn save_region_file_dialog() -> Option<PathBuf> {
            Self::region_file_dialog().save_file()
        }
        ...
        pub fn write_regions(&self, path: PathBuf) {
            ...
            f.write_all(&region_string.as_bytes())
...
mod tests {
    ...
    fn test_as_string() {
        ...
        assert_eq!(
            region_as_string, "",
            "Should not contain any enabled regions"
        )
    }
    ...
    fn test_write_regions() {
        ...
        let file_contents =
            std::fs::read_to_string(temp_file.path()).expect("Failed to read temporary file");
    ...
    fn test_set_active_regions() {
        ...
        assert!(
            !regions_vm.regions.get_mut(&region).expect("").0,
            "Region 6101000 should be locked"
        );
        regions_vm.set_active_regions(&vec![key]);
        assert!(
            regions_vm.regions.get_mut(&region).expect("").0,
            "Region 6101000 should be unlocked"
        );
        assert_eq!(
            regions_vm
                .regions
                .values()
                .filter(|&&(is_active, _, _, _)| is_active)
                .count(),
            1,
            "Only one region should be unlocked"
        );
