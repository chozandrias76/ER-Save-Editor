pub mod regions {
    ...
    use crate::{
        ...
        vm::regions::regions_view_model::RegionsViewModel,
        App,
    };
    ...
    fn export_file(ui: &mut Ui, steam_id_presence: bool, app: &mut App) {
        let export_button = egui::widgets::Button::new(egui::RichText::new(format!(
            "{} Export Regions",
            egui_phosphor::regular::DOWNLOAD_SIMPLE
        )));

        if ui.add_enabled(steam_id_presence, export_button).clicked() {
            let path = RegionsViewModel::save_region_file_dialog()
                .expect("Should provide a path to a new or existing file");
