pub mod updater {
            ...
            if let Some(confirmed) = self.update_confirmed {
                if confirmed {
                    ...
                    let current_exe_path =
                        Self::download_and_replace_current_binary(&self.browser_download_url);
                    ...
                    match current_exe_path {
                        Ok(path) => {
                            ...
                            Command::new(path)
                                .spawn()
                                .expect("Program did not restart correctly");
                        }
                        Err(e) => {
    ...
    impl UpdaterApp {
        fn download_and_replace_current_binary(
            download_url: &str,
        ) -> Result<String, Box<dyn std::error::Error>> {
            let current_exe_path = env::current_exe()
                .expect("Could not find the currently running exe")
                .display()
                .to_string();
        ...
        pub fn check_for_updates() -> Result<(), Box<dyn std::error::Error>> {
            ...
            if latest_version != local_version {
                println!(
                    "An update is available! Your version: {}, latest version: {}",
                ...
                if let Some(asset) = latest_release
                    .assets
                    .iter()
                    .find(|a| a.browser_download_url.ends_with("er-save-editor.exe"))
                {
